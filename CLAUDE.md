...
| `#/division-bells` | `DivisionBells` | How far apart are two 2-D Gaussians P/Q, two lenses on one scene: **Mahalanobis separation** (mean gap in σ-units, directed-in-Q + pooled-symmetric) and **Kullback–Leibler divergence** (asymmetric). Payload = the exact split `KL = ½·d_M²(means) + ½·Σ(λ−1−lnλ)` (cov mismatch): matching the two σ-ellipses zeroes the cov term so `KL = ½·d_M²` exactly (*Match shapes* preset), differing shapes make KL asymmetric (*Concentric* → dₘ=0 yet KL>0). Immersive SVG plane (draggable means + σ-ellipses + difference vector), theme-token colored (P=`--data-1`, Q=`--data-2`). Pure engine `gaussian2d.ts` (θ,σ₁,σ₂ Gaussian; closed-form KL/Bhattacharyya/Hellinger + eigenvalue KL decomposition + numeric TV/overlap/Bayes-error — the whole divergence family, unit-tested). Gallery tile echoes the Pink Floyd *Division Bell* cover (two facing bell profiles, lit gap = the divergence). Wave 1 ships Mahalanobis + KL; Bhattacharyya/Hellinger + Bayes-error/TV (numeric; Bayes-error as the operational anchor) staged next — the engine already computes them. Possible Wave-4: Wasserstein-2 (Bures closed form for Gaussians) as a geometry-aware lens |
