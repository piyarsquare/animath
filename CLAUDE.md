...
└── src/
    ...
    │   └── Argand/              # entry-point app for complex numbers: drag two numbers, watch add (tip-to-tail slide) vs multiply (spiral — angles add, lengths multiply) + commutativity; five Views (Number · Curve · Plane · Repeat · Mean) — Plane morphs the whole coordinate grid over a ghost identity grid, Repeat shows multiplication as repeated addition (m copies of a + n copies of j·a), Mean contrasts arithmetic vs geometric (vs harmonic) midpoints; a System slider p=j² unifies complex/dual/split-complex (rotation/shear/boost, with the x²−p·y²=1 unit curve + null cone), generalizing multiplication across every View. Play paces by arc length (constant pen speed) with labeled stops. Generalized algebra (mulG/powRealG/lerpMulG/means) in complexOps.ts, SVG/DOM plane in ArgandPlane.tsx. Successor-in-progress to Plane Transform
    │
