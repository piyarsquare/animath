...
└── src/
    ...
    │   ├── TreesAndNets/        # phylogenetics from data: an editable distance matrix → Neighbor-Joining tree + circular split network ("the nets") + SplitsTree split-graph + split weights (default **Nets** layout). Four top-bar modes: **Nets** (matrix↔tree↔net↔weights, all cross-linked by one shared selection) · **Build** (the inverse — weight a tree's branches or a circular order's chords and the matrix is *generated* as d = Σ separating weights; views/BuildPanel.tsx + lib/buildMetric.ts: candidateSplits/fitWeights/metricFromItems via NNLS) · **Fibers** (the associahedron × (n−3)-cube tree-space explorer, a secondary layout) · **Run** (stepwise NJ/NeighborNet with the per-step **Q matrix** showing *why* each pair joins, and NeighborNet's order-blocks locking once a merged block exceeds size 2; views/AlgorithmView.tsx + computeNeighborJoiningTrace/computeNeighborNetTrace). Distance-driven engine ported from the private quantum-tree (`map.js`): lib/{metric,trees,neighborJoining,splitWeights,orders,splitGraph,buildMetric}.ts — four-point support, tour energy, Saitou–Nei NJ (+trace), NNLS circular split weights, Levy–Pachter ordering, SplitsTree convex-hull split graph. Views in views/{MatrixEditor,NetViews,AlgorithmView,BuildPanel}.tsx, all theme-tracking via views/themeColors.ts (`useNetColors`); legacy lib/{associahedron,mosaic}.ts still drive the fibers. Math is unit-tested (vitest, lib/__tests__/{engine,recovery,trace,splitGraph,buildMetric}.test.ts: random-tree/CDM round-trip recovery, NJ/NeighborNet traces, split-graph, builder fit). Remaining quantum-tree port (deferred): the trees×orders correspondence map, energies on the fibers, the quartet evidence plane + two-route (tree-like vs net-like) readout, fiber walks, perturbation studies
...
| `#/trees-and-nets`   | `TreesAndNets`   | Phylogenetics from a distance matrix: Neighbor-Joining tree + circular split network (the "nets") + SplitsTree split-graph + split weights. Four modes: **Nets** · **Build** (weight edges → generate the metric) · **Fibers** (associahedron × cube tree-space) · **Run** (stepwise NJ/NeighborNet with the Q-matrix "why"). Port of the private `quantum-tree` metric/evidence engine |
