...
└── src/
    ...
    │
    │   └── NumberPlane/          # one expression, three arithmetics: three plots (j² = −p·0·+p) of the same |z|=r / αz+β / z² under each plane's multiplication, single p dial; first consumer of Argand/numberPlanes.ts; Beat-4 comparator, may replace Argand
...
| `#/number-plane`     | `NumberPlane`    | One expression, three arithmetics: a single panel with three plots (j² = −p, 0, +p) rendering the same expression — \|z\| = r level sets, αz + β (drag α/β on any plot), z² — under each plane's own multiplication, on the dormant-no-more `numberPlanes.ts` engine; a single p dial morphs the outer planes while the dual holds still. The Beat-4 comparator of the Number Planes story; may grow to replace Argand Plane |
