...
└── src/
    ...
    │   ├── TreesAndNets/        # phylogenetics: distances ↔ a Neighbor-Joining tree + circular split network ("the nets") + SplitsTree split-graph + split weights, all cross-linked by one shared selection. Four top-bar modes (Build is the default front door): **Build** (the centerpiece — *fatten a tree into a net*: leaves in a fixed cyclic order make every tree edge a non-crossing arc split; a **Fatten** slider per internal edge adds the *crossing* companion split, opening a box, so a tree becomes a CDM; views = NJ tree (splits) + SplitsTree net + the generated matrix; lib/fattenTree.ts: balancedTreeEdges/companionSide/fattenedMetric, controls in views/BuildPanel.tsx `FattenControls`) · **Nets** (edit the distance matrix directly → tree+net+weights) · **Run** (stepwise NJ/NeighborNet with the per-step **Q matrix** showing *why* each pair joins, and NeighborNet's order-blocks locking once a merged block exceeds size 2; views/AlgorithmView.tsx + computeNeighborJoiningTrace/computeNeighborNetTrace) · **Fibers** (the associahedron × (n−3)-cube tree-space explorer, kept last/de-emphasized). Distance engine ported from the private quantum-tree (`map.js`): lib/{metric,trees,neighborJoining,splitWeights,orders,splitGraph,fattenTree}.ts — four-point support, tour energy, Saitou–Nei NJ (+trace), NNLS circular split weights, Levy–Pachter ordering, SplitsTree convex-hull split graph, fatten-a-tree CDM builder. Views in views/{MatrixEditor,NetViews,AlgorithmView,BuildPanel}.tsx, all theme-tracking via views/themeColors.ts (`useNetColors`); legacy lib/{associahedron,mosaic}.ts still drive the fibers. Math is unit-tested (vitest, lib/__tests__/{engine,recovery,trace,splitGraph,fattenTree}.test.ts: random-tree/CDM round-trip recovery, NJ/NeighborNet traces, split-graph, fatten-tree additivity + box-opening). Remaining quantum-tree port (deferred): the trees×orders correspondence map, energies on the fibers, the quartet evidence plane + two-route (tree-like vs net-like) readout, fiber walks, perturbation studies
...
| `#/trees-and-nets`   | `TreesAndNets`   | Phylogenetics: a Neighbor-Joining tree + circular split network (the "nets") + SplitsTree split-graph + split weights. Four modes: **Build** (default — *fatten a tree into a net*, see NJ tree + SplitsTree net + generated matrix) · **Nets** (edit a distance matrix → tree+net) · **Run** (stepwise NJ/NeighborNet with the Q-matrix "why") · **Fibers** (associahedron × cube tree-space, de-emphasized). Port of the private `quantum-tree` metric/evidence engine |
