...
└── src/
    ...
    │   ├── TreesAndNets/        # phylogenetics as a **tabbed app of independent domains** — deliberately NO crossover: each tab is one transform (input → pure fn → output), rendering its OWN `<Workspace>` (own appId) with its own state and its own teaching explainer. `TreesAndNets.tsx` is just a tab router; the tabs live in `tabs/` (each a self-contained mini-app; `tabs/common.tsx` holds APP_ID/TABS/NavProps + `useMatrixState`/`MatrixControls`). The shared *atom* is conceptual — a **weighted split system** — of which a distance matrix, a tree, and a net are three views. Tabs: **Matrix → Tree** (edit D → Neighbor-Joining), **Tree → Matrix** (branch-length sliders → additive metric + generated matrix), **Matrix → Net** (D → NeighborNet split network + weights), **Circular sums** (the *tour energy* — a circular summation of D — plotted across every circular order, with the selected order traced; views/ValuePlot.tsx = ValuePlot + CircleTour), **Run** (stepwise NJ/NeighborNet with the per-step **Q matrix** showing *why* each pair joins). Engine ported from the private quantum-tree (`map.js`): lib/{metric,trees,neighborJoining,splitWeights,orders,splitGraph,fattenTree}.ts — four-point support, tour energy, Saitou–Nei NJ (+trace), NNLS circular split weights, Levy–Pachter ordering, SplitsTree convex-hull split graph, balanced-tree builder; unit-tested (lib/__tests__/{engine,recovery,trace,splitGraph,fattenTree}.test.ts). Views in views/{MatrixEditor,NetViews,AlgorithmView,ValuePlot,BuildPanel}.tsx, theme-tracking via views/themeColors.ts (a per-skin **weight colormap**). The associahedron × (n−3)-cube **Fibers** explorer (lib/{associahedron,mosaic}.ts) is parked — removed from the tab bar, re-addable as a tab. Next: the DAG of tab inputs/outputs; doc-first storylines feeding new tabs (e.g. Tree → Net "fatten", quartet/four-point sums)
    │   └── SolidWorlds/         # walk inside a closed flat 3-manifold from one glued cube (3D successor to Polygon Worlds); catalog now 8 of the 10 platycosms (3-torus, half/quarter-turn, both amphicosms, both amphidicosms, Hantzsche–Wendt); developing-map cover engine (coverEngine.ts), chirality HUD (original · rotated · mirrored). Third person uses a **cutaway clip plane** (a single world plane a gap in front of the camera — the **Cutaway** slider sets that gap as a fraction of the camera→character distance, clamped just short so it never reaches the character — perpendicular to the camera→character line; every cover material shares it by reference except the floor, which stays whole; avatars are outside coverRoot so never clip) so the near wall the camera pokes through is clipped away and the avatar is never buried; empty in first person. Invariants: H₁ from the deck group's abelianization (lib/freeness.ts: point group + Reidemeister–Schreier lattice → Smith form) — the authoritative, screw-safe H₁ — plus a free-action test certifying a genuine manifold (vs a cone-point orbifold); the cube cell complex (lib/homology.ts) is kept for χ and as an independent cross-check — all 8 worlds are now dual-verified (the 2026-06-20 screw fix: glue by the whole in-cube orbit + a finer subdivision for screw worlds; see SolidWorlds/SCREW_BUG.md). View panel also has a **Decor** switch (Diagnostic ↔ Rooms): the Rooms mode (decor/rooms.ts) furnishes the fundamental cube as a recognizable room — rug, desk+lamp, fireplace (+logs/ember glow), framed picture, ceiling chandelier, a Hello sign over the mantel, plus an open-fronted bookshelf on the +x side wall (colored book spines on every shelf, a potted plant, and a small glass Klein bottle ornament — kleinBottleGeometry, the classic immersion) and a wardrobe on the +z wall so the formerly-bare back walls are furnished too (bookshelf on a side wall keeps it clear of the default third-person camera, which sits behind the walker near the +z wall) (all emissive-only "light", no real point lights, which would multiply across the cover and fight the holonomy-symmetrized lighting) — inside **solid-but-faintly-translucent walls** (faint axis tint X warm · Y green · Z blue; opacity is the **Wall opacity** slider, default 0.84, with depthWrite ON so the tiled panels don't flicker, yet a hint of the copies glows through — engine keeps live wall-material refs so the slider updates without a rebuild) whose room-to-room connections are **off-center archways**: each of the three −axis faces is a solid panel (ShapeGeometry) with one arched opening (the floor/ceiling a hole), framed by an extruded **arch-molding casing** (jambs + arched header proud of the wall; a low torus rim around the trapdoor) so the opening reads as built architecture; each wall also carries a steel-framed **ceiling duct** (a slot running up to the ceiling in the opposite corner, open at the top edge as the arch is open at the floor) — distinct from the arched doorways — so worlds that flip the room top↔bottom keep a foot-level transit opening (the gluing carries the ceiling duct down to floor level in the flipped copy, so you cross through the duct instead of clipping blank wall while the arch hangs from the ceiling); the +axis faces are supplied by neighbors via the cover instancing, so a far wall's opening is your near opening carried by that pairing's gluing — off-center so the turn/mirror is visible (centered would be fixed by it), and in worlds tipping vertical↔horizontal the ceiling hole lands where a wall arch was (chandelier on a wall). No g-math in the decor: drawing only the −axis faces lets the deck instancing transport them. Solid walls stage the surprise (you don't see the next room until you step through). Textures in textures.ts (rugTexture/pictureTexture/signTexture). Default cover depth is 1 (a calm single room; slider →10 for the deep hall) — depth 3 read as far too busy. Earlier passes: faint-frame "doorless" walls (arrows on walls — removed as unhelpful), and before that interior architecture (glass walls/spiral stairs — too cluttered). Visual/design layer only, no engine-math change
    │   └── Argand/              # entry-point app for complex numbers, built around the affine map f(z)=α₁·z+α₀ (the complex cousin of y=mx+b): drag the slope α₁ and shift α₀ (lockable, color-coded to the equation), feed f a Point/Shape/Grid (top-bar pills), watch the two honest legs (×α₁ spiral, then +α₀ slide) closed into a loop (back via the all-at-once diagonal) and the fixed point z*=α₀/(1−α₁); "View from z*" recenters so f becomes a pure spiral, and Iterate draws the orbit log-spiral into/out of z* (Point feed). A System slider p=j² (persistent bottom pill: Complex/Dual/Split) runs the same line through complex/dual/split-complex (rotation/shear/boost, x²−p·y²=1 unit curve + null cone). a Degree pill extends f to quadratics α₂z²+α₁z+α₀ (pink α₂ handle): the grid bends (sampled, not endpoint-mapped), two fixed points + a critical point appear, and the Point feed animates Horner's method as a chain of ×z spirals + coefficient slides ("scaled effects modulated by p"); Shape/Grid morph the α₂ term in via a degree-ramp. Immersive fill-the-screen plot with a bottom control HUD (feed switcher + t and j² scrubbers/play, lifted above the phone dock); Play paces by arc length. Poly engine (polyEval/hornerAt/polyFixedPoints/criticalPoint/polyRampAt) + affine + generalized algebra (mulG/powRealG) in complexOps.ts; a math-first foundation designed from the three classes (Spin/Shear/Boost over p=j², generic algebra primary + the polar layer modulus/arg/fromPolar with honest domains + a `plane(p)` strategy, all unit-tested) in numberPlanes.ts (`__tests__/numberPlanes.test.ts`); SVG plane in ArgandPlane.tsx. A standalone entry point to "arithmetic on the plane" (the *Number Planes* idea — the complex plane as one of a family; Spin/Shear/Boost); it complements, and is NOT a replacement for, Plane Transform or the complex plane
    │   └── CountingTheWays/     # why a Bessel function gives the conditional probabilities behind a Skellam (the difference of two independent Poisson counts — e.g. microsatellite repeats gained − lost): the (gains, losses) lattice with the swept k-diagonal revealed rung-by-rung as the Bessel sum I_{|k|}(2√μ₁μ₂), the color-linked factored formula, and the conditional-over-rungs readout (each conditional = one Bessel term ÷ the Bessel sum). Rates set directly or from a softplus length-law f(L)=softplus(a+bL) (two arms, read at length L). Top-bar modes Explain (Play runs a narrated tutorial that builds the whole matrix — Poisson margins → every cell as a product → the k-diagonal summed rung-by-rung onto P(K=k)) · Lab (cataloged simulator: each run draws a sample, recovers μ̂ by method-of-moments, and logs a row to a results table to compare seeds/sizes/rates). Pure math engine in skellam.ts (log-space Poisson, softplus law, modified Bessel series + per-term accessor, honest diagonal-sum pmf, seeded sampler, moment fit). CSS/DOM + SVG, no WebGL
    │   └── DivisionBells/       # how far apart are two 2-D Gaussians P/Q — Mahalanobis separation × KL divergence as two lenses on one scene (named for the Pink Floyd *Division Bell*: divergence + bell curves; gallery tile in previews.tsx echoes the two-facing-heads cover, gap = the divergence). The teaching payload is the exact identity `KL(P‖Q) = ½·d_M²(means, in Q's metric) + ½·Σ(λ−1−lnλ)` over eigenvalues λ of Σ_Q⁻¹Σ_P — so *Match shapes* zeroes the covariance term and KL collapses to exactly ½·d_M², while differing shapes switch the cov term on and make KL asymmetric (*Concentric* → means equal, dₘ=0, yet KL>0). Immersive SVG plane (DivisionBells.tsx inlines the plane: draggable P/Q means, 1σ/2σ ellipses from the (θ,σ₁,σ₂) parametrization, density fill, mean-difference vector), theme-token driven (P=`--data-1`, Q=`--data-2`, no hardcoded scene color; SVG reads `var(--…)` directly so no canvas token-reads needed yet). Analyze panel: two-bar KL Breakdown (mean-shift vs cov-mismatch, both non-negative via the eigenvalue form) + StatGrid (KL both directions nats·bits, Mahalanobis directed-in-Q **and** pooled-symmetric in σ-units, asymmetry). Pure engine `gaussian2d.ts` computes the **whole divergence family up front** — closed-form Σ/Σ⁻¹/Σ^{±½}/det, pdf+logPdf, Mahalanobis, KL + decomposition, Bhattacharyya→Hellinger, Bhattacharyya bound, and **numeric** TV/overlap/Bayes-error (no Gaussian closed form: the decision boundary is a conic) — unit-tested `__tests__/gaussian2d.test.ts` (closed forms vs numeric integration both ways, the exact collapse, Bhattacharyya + Pinsker bounds, degenerate flooring; 18 tests). **Shipped (all waves):** Mahalanobis + KL (the exact-collapse payload) · the yardstick family (Bhattacharyya, Hellinger, TV, Bayes error, Wasserstein-2) behind a progressive-disclosure *All measures* layout with **Bayes error the operational anchor** ("how confusable are the two bells") · the canvas field (density / KL-integrand / decision-overlap) · the whitening view. `measures.ts` is a stateless presentation registry (math stays in `gaussian2d.ts`); KL + Mahalanobis stay bespoke. Three-hats design review + the divergence-family scoping live in `docs/sessions/progress/modest-cannon-umd49e/`. CSS/DOM + **SVG over a 2-D canvas** (the field heat), no WebGL
    │   └── SplitDecision/       # a GA evolves a two-way split of a binary matrix; seven selection-score "judges" (scores.ts), three reproductive "worlds" incl. two sexes transmitting intact halves (evolve.ts operator model), sustained-optimum "reached", lab/sweep.ts pure sweep for the Phase 3 worker; src/lib/rng.ts is the first shared mulberry32
    │
...
| `#/trees-and-nets`   | `TreesAndNets`   | Phylogenetics as a **tabbed app of independent domains** (no crossover; each tab is one transform with its own state/`<Workspace>`/explainer): **Matrix → Tree** (Neighbor-Joining) · **Tree → Matrix** (branch lengths → additive metric) · **Matrix → Net** (NeighborNet split network) · **Circular sums** (tour energy plotted over every circular order) · **Run** (stepwise NJ/NeighborNet, Q-matrix "why"). Shared atom: a weighted split system. Port of the private `quantum-tree` engine; the associahedron×cube *Fibers* explorer is parked |
...
| `#/argand`           | `Argand`         | Drag two complex numbers; arithmetic as geometry (add slides, multiply spirals). A standalone entry point to "arithmetic on the plane" (complex numbers as one of a family of *Number Planes*); complements Plane Transform, not a replacement |
...
| `#/counting-the-ways` | `CountingTheWays` | Why a Bessel function gives the conditional probabilities behind a Skellam (difference of two Poisson counts): the (gains, losses) lattice with a narrated Play-tutorial that builds the whole matrix and sums the k-diagonal into the Bessel value (Explain), plus a cataloged simulator that recovers μ̂ from synthetic samples (Lab); optional softplus length-law rates |
| `#/division-bells` | `DivisionBells` | **(Storeroom — parked; route live)** How far apart are two 2-D Gaussians P/Q, two lenses on one scene: **Mahalanobis separation** (mean gap in σ-units, directed-in-Q + pooled-symmetric) and **Kullback–Leibler divergence** (asymmetric). Payload = the exact split `KL = ½·d_M²(means) + ½·Σ(λ−1−lnλ)` (cov mismatch): matching the two σ-ellipses zeroes the cov term so `KL = ½·d_M²` exactly (*Match shapes* preset), differing shapes make KL asymmetric (*Concentric* → dₘ=0 yet KL>0). Two top-bar **modes**: **On a line** (default — the "felt" 1-D view: two bell curves on one axis where Mahalanobis = the peak gap read off a **σ-ruler** in bell-widths, and the divergence = the **shaded overlap lens**, area = overlap, Bayes error = ½·overlap at equal priors, crossing = decision boundary; `Bells1D` + pure `gaussian1d.ts`, unit-tested) · **On the plane** (the full 2-D version, kept in reserve). Immersive SVG plane (draggable means + σ-ellipses + difference vector) over a **canvas field** (Field selector: density · signed KL-integrand `p·log(p/q)` · classify = winner tint + the lit Bayes-error band at the decision boundary), theme-token colored (P=`--data-1`, Q=`--data-2`; canvas via `useThemeTokens`). A **Whiten into P/Q** toggle maps the plane by Σ_ref^{−½} so the reference bell is a unit circle and Mahalanobis reads as ordinary distance. **Yardstick family** panel (`measures.ts` registry): Bayes error (the operational anchor) · TV · Hellinger · Bhattacharyya · Wasserstein-2 · KL, with bounded/metric/numeric tags, a prior π(P) slider, and live relationship checks (Pₑ=½(1−TV), Bhattacharyya bound Pₑ≤½·BC, Pinsker TV≤√(KL/2)). Pure engine `gaussian2d.ts` (θ,σ₁,σ₂ Gaussian; closed-form KL/Bhattacharyya/Hellinger/**Wasserstein-2 (Bures)** + eigenvalue KL decomposition + numeric TV/overlap/Bayes-error — the whole family, unit-tested, 21 tests). Gallery tile echoes the Pink Floyd *Division Bell* cover (two facing bell profiles, lit gap = the divergence) |
| `#/split-decision` | `SplitDecision` | **(Storeroom — parked; route live)** A genetic algorithm evolves a two-way split of a binary matrix (rows = cells, columns = variants). Genotype = row/column inclusion probabilities; fitness = one of seven **judges** (`scores.ts` registry over a block table: Cut and Run · Hamming's Full Count · Pearson's Squint · Shannon's Onesie · The Full Bernoulli · Newman's Leftovers (Barber's Q) · Occam's Invoice (MDL relative to the no-split code, so every judge gives 0 to "no split")) of one **sampled** cut (or the rounded genome). The experiment is the reproductive rule (`evolve.ts` operator model: tournament / mate / reflecting mutate / evaluate; the three Worlds as a `RULES` table): **Muller's Monastery** (clonal) · **Hardy–Weinberg Mixer** (uniform crossover) · **Potter–De Jong Prom** (two sexes, each transmitting one half intact; a child mutates only the half it will transmit). The **Sex quota** pill relaxes the composition: **Enforced** (exactly round(N·r) row-sex every generation, so runs are comparable) or **Drifting** (each birth its own draw, so the realized ratio wanders). Neither clamps any more, so a sex can be missing entirely — and a generation with only one sex present reproduces **parthenogenetically**: one parent, both halves transmitted, both mutated (the whiptail-lizard outcome; `rowShare` on GenStats reports the realized composition and the readout flags it). With a fixed ratio that is a fluctuation, not a fate — the lost sex is back in the next draw; a *heritable* ratio is what would let sex go for good. Watch mode: the **Arena** (DOM grid, rows/columns reorder by the canonicalized population-mean genome with transform transitions, one cross-block tint, edge bars + IQR whiskers, planted markers, paint mode), **Trace** (best-rounded/mean fitness with planted + exact-optimum reference lines; three entropy curves: sampled · rounded twin · neutral twin), **Population** (genome strips on a canvas, sex by presence/absence tick), **Linkage** (`linkage.ts` + `views/Linkage.tsx`: the (m+n)² correlation matrix between loci across the population, divergent colormap, ordered like the Arena, with mean |r| per block against the independence baseline √(2/π)/√N. The picture is **block-first**: each block is washed neutrally by how far its mean |r| runs above that baseline and every group boundary is ruled, so a rule's signature reads at a glance (all blocks lit · none lit · within-half only); cells carry the sign on a **fixed** ±0.5 scale — deliberately not auto-fitted, since normalizing per frame would make the Mixer's noise as loud as the Monastery's signal and destroy the very comparison the view exists for — and the diagonal is muted, being 1 by definition — the view where the three rules differ most: the Mixer flattens everything (linkage equilibrium, Geiringer 1944), the Monastery keeps every block high, and the Prom keeps the within-half blocks while collapsing the ACROSS block, which is why it cannot make coordinated row-and-column moves; unit-tested against an independent Pearson implementation). A **Payoff** axis (Judge panel) runs the whole thing a second way: **Shared judge** (every individual scored by the judge — the default, and the only thing the Lab sweeps) or **Sexed yields**, where each gender is scored by its own cross block instead — the **row** gender reads its rows as inclusion and its columns as exclusion and takes the ones in R₁×C₂, the **column** gender reads them the other way and takes R₂×C₁ (`sexedYield` in scores.ts). The two blocks sum to the bipartite edge count, so the genders divide one pot: they want the same cut to be a good cut, and opposite sides of every locus. A **Yield** sub-pill picks how a gender's capture is scored: **Density** (default — its ones over the **cells of its own block**), **Count** (its ones as a share of every 1 in the matrix) or **Excess** (share of the ones minus share of the cells). The three give different games, all measured. Under **count** there is **no degenerate guard**, deliberately — taking more is always better, so each gender's individually-best move is "include my whole half" and the optimum is a cut that does not split at all; the worlds resolve it oppositely (clonal → conquest, one gender at 100% and the pot fully captured; the two-sex Prom → mutual ruin, both near 0 with the pot empty). Under **density** the empty cells come with the ones, so the runaway is gone and every outcome is a proper interior split — and the regime is then set by the **signal**: on a clean checkerboard both genders and the pot peak together at the true split (clonal, signal 1: |R₁|=|C₁|=5, both densities 1.0, **captured 1.00**), while on a noisy one they shrink onto a single row and column each, keep a decent density, and let the pot collapse (Prom, signal 0.6, k=8: |R₁|=|C₁|=1, densities ≈0.78, **captured 0.29**). A second tragedy, hoarding rather than swallowing. The third option, **Excess** (its share of the ones minus its share of the cells — the ones beyond what a block that size holds by chance, i.e. the per-block term of Newman's Leftovers), was added on 2026-09-18 to clear both barriers: swallowing the matrix cancels to exactly 0 and hoarding one cell scores ≈1/T, so a block only scores by being denser than chance *and* big. Measured (10×10, N=128, k=8, gen 600): clean signal → the true split in both worlds, +25 pts each, captured 1.00; noisy (0.6) → **no** swallow and **no** hoard, but the worlds still differ — the clonal world converges on the winning gender's exact private optimum (a 7×7 block, +18 pts, verified by enumeration) and pushes the other below chance (−1 pt, captured 0.76), while the two-sex Prom settles on an interior 5×5-ish split where both score (+11 / +7 pts, captured 0.65–0.67 against 0.71 at the planted cut, stable to 3000 generations, same shape on 4 seeds). What is left once the trivial barriers go is winner-takes-all under one parent versus a two-gender compromise. Under sexed payoff the Trace plots the two genders' yields instead of best/mean, the Readouts add a *two genders* block (yields · captured · classes included) and drop the judge's optimum/planted/reached rows, and the Lab forces the shared judge — two yields are not one objective to reach. Exact optimum by enumeration for m+n ≤ 20; "reached" = sustained rounded optimum. Fixtures quoted from the September 2026 monograph (8×10 complete/sparse, the 90-matrix 4×4, the page-50 trap — a strict single-flip local optimum only under modularity). Pure engine unit-tested (`__tests__/{scores,evolve}.test.ts`, 39 tests incl. seed-batch convergence and a canalization contrast against the neutral curve). **Lab** mode (top-bar pill): a worker-pool sweep (`lab/{sweep,worker,pool}.ts`; one run per job, main-thread fallback) over planted signal × rule × seed — the Lab plants its **own** matrices (`labM`/`labN` sliders, separate from the Watch Matrix panel, so turning the watched matrix past the m + n ≤ 20 enumeration limit can no longer disable a sweep) and the catalog stores per-cell **summaries**, not raw rows — or the **Escape the trap** preset (page-50 fixture, every individual started at the trap cut, judge forced to modularity); `views/Sweep.tsx` plots reached-by-G_max across signal, survival curves at a chosen level, a hypotheses box (predicted → observed, H1 Mixer > Monastery at strong signal · H2 Prom slower than Mixer · H3 trap escape Monastery ≥ Mixer > Prom), and a persisted catalog. Plan + three-hats + progress in `docs/sessions/progress/elegant-ride-e1q79l/`. A **Locus order** pill (Cells panel) lays both pictures out three ways: **Live** (sort by the population mean — the reorganizing IS the discovery, at the cost of positions that move), **Planted** (grouped by the true cut and frozen for the whole run, so nothing moves and what changes is the color filling into the blocks; hidden when the instance has no planted cut) and **Stored** (the shuffled generation order, no grouping). `LocusOrder` carries the block boundaries, so the Arena and the correlation matrix always rule in the same places. Watch mode ships three authored layouts that genuinely rearrange the windows (Essentials · Linkage · Population) beside the builtin Compact/Everything — a layout that overrides no view rect leaves every window where it was and reads as a broken menu. The **operator dials are live** (tournament k, μ, σ, sex ratio, sex quota): `useWatchLoop` reads them through a ref each step and leaves them out of its rebuild key, so turning selection up on a drifted population canalizes it in place and the trace bends where you turned it; the matrix, judge, fitness, payoff, rule, N and seed still rebuild (a population evaluated under one judge is not a population under another). The k slider runs 1–16 with labeled stops (drift · 8 · hard) and is clamped at the state boundary so a stored out-of-range value can never reach the engine while the label says otherwise. The Watch loop is deliberately three-cadence (generations free, React ≤20 Hz, population-level stats ≤4 Hz) after a profile showed it was reconciler-bound; the Population and Linkage views are canvas for the same reason. Next: real-device pass, more sex rules (within-sex recombination, mate choice), the mother's-curse readout, and the monograph's statistics as a later chapter |
...
**Storeroom.** A `catalog.ts` META entry can set `storeroom: true` to park a card
in the gallery's **Storeroom** — a collapsed, de-emphasized section at the bottom
(out of the main grid and the category filter), for experiments that aren't ready
and for retiring old gallery elements. The route stays live (the card is still
clickable from the Storeroom); only its gallery prominence drops. Prefer this over
deleting a card when an app should recede but stay reachable.
