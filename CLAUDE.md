...
└── src/
    ...
    │   ├── TreesAndNets/        # phylogenetics from data: an editable distance matrix → Neighbor-Joining tree + circular split network ("the nets") + split weights (default **Nets** layout), with the associahedron × (n−3)-cube tree-space **Fibers** demoted to a secondary layout. Distance-driven engine ported from the private quantum-tree (`map.js`): lib/{metric,trees,neighborJoining,splitWeights,orders}.ts — four-point support, tour energy, Saitou–Nei NJ, NNLS circular split weights, Levy–Pachter ordering, compatibility (vitest in lib/__tests__/engine.test.ts). Views in views/{MatrixEditor,NetViews}.tsx; legacy lib/{associahedron,mosaic}.ts still drive the fibers. Remaining quantum-tree port (deferred): the trees×orders correspondence map, energies on the fibers, the quartet evidence plane + two-route (tree-like vs net-like) readout, fiber walks, metric builder, perturbation studies
...
| `#/trees-and-nets`   | `TreesAndNets`   | Phylogenetics from a distance matrix: Neighbor-Joining tree + circular split network (the "nets") + split weights (default **Nets** layout), with the associahedron × cube tree-space **Fibers** as a secondary layout. Port of the private `quantum-tree` metric/evidence engine |
