...
└── src/
    ...
    │   └── Argand/              # entry-point app for complex numbers, built around the affine map f(z)=α₁·z+α₀ (the complex cousin of y=mx+b): drag the slope α₁ and shift α₀ (lockable, color-coded to the equation), feed f a Point/Shape/Grid, watch the two honest legs (×α₁ spiral, then +α₀ slide) and the fixed point z*=α₀/(1−α₁). A System slider p=j² runs the same line through complex/dual/split-complex (rotation/shear/boost, x²−p·y²=1 unit curve + null cone). Play paces by arc length with 3 stops (z·α₁z·f(z)). Affine engine (affine/affineAt/fixedPoint) + generalized algebra (mulG/powRealG) in complexOps.ts, SVG plane in ArgandPlane.tsx. Designed to grow to quadratics/polynomials. Successor-in-progress to Plane Transform
    │
