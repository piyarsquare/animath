...
└── src/
    ...
    │   └── Argand/              # entry-point app for complex numbers: drag two numbers, watch add (tip-to-tail slide) vs multiply (spiral — angles add, lengths multiply) + commutativity; three subjects (Number · Curve · Plane), the Plane chapter morphing the whole coordinate grid by z↦z·b / z+b over a ghost identity grid; Play paces by arc length (constant pen speed) with labeled stops. Pure ops in complexOps.ts, SVG/DOM plane in ArgandPlane.tsx. Successor-in-progress to Plane Transform
    │
