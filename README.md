...
10. **[Trees and Nets](https://piyarsquare.github.io/animath/#/trees-and-nets)** – phylogenetics from data: edit a distance matrix and watch it become a Neighbor-Joining **tree** and a circular split **network** (the "net"), where crossing chords reveal structure no single tree can hold. A *Run* mode plays Neighbor-Joining and NeighborNet **step by step** — the Q matrix shows *why* each pair joins, and the circular order locks in block by block. A *Build* mode runs it backwards: weight a tree's branches or a circular order's chords and the distance matrix is *generated*. A secondary *Fibers* layout keeps the tree-space geometry — the associahedron of triangulations and the cube of orders. Port of the private `quantum-tree`.
