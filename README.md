...
5. **[Trinary System](https://piyarsquare.github.io/animath/#/trinary)** – drop a planet into a chaotic three-star system and watch its future diverge; an in-app **Lab** tab runs thousands of worlds and maps their fates into fractal "destiny" portraits and statistics.
...
7. **[Stable Matching](https://piyarsquare.github.io/animath/#/stable-matching)** – a rebuilt Gale–Shapley lab: tune each side's consensus, watch the proposer advantage, sweep the whole consensus plane, and browse the lattice of stable matchings.
8. **[Polygon Worlds](https://piyarsquare.github.io/animath/#/polygon-worlds)** – one decorated square, four worlds: glue its edges and walk a torus, Klein bottle, projective plane or sphere in first person.
9. **[Trees and Nets](https://piyarsquare.github.io/animath/#/trees-and-nets)** – phylogenetics as a set of **independent tabs**, each one transform with no crossover: **Matrix → Tree** (edit distances → Neighbor-Joining tree), **Tree → Matrix** (set branch lengths → the additive distance matrix), **Matrix → Net** (distances → a SplitsTree split network, where crossing splits open boxes), **Circular sums** (the *tour energy* plotted across every circular order of the leaves), and **Run** (play Neighbor-Joining / NeighborNet step by step, the Q matrix showing *why* each pair joins). They all stand on one shared atom — a weighted split system, of which a matrix, a tree, and a net are three views. Port of the private `quantum-tree`.
10. **[Solid Worlds](https://piyarsquare.github.io/animath/#/solid-worlds)** – walk *inside* a closed 3-manifold built from one glued cube — a single room that repeats forever. Walk an orientation-reversing loop in the Klein-bottle × circle world and the whole world, sign and your own footprints come back mirror-reversed (the 3D successor to Polygon Worlds).
...
14. **[Split Decision](https://piyarsquare.github.io/animath/#/split-decision)** – a population evolves a two-way split of a binary matrix (cells × variants) under your choice of judge — cut count, Hamming, chi-square, two mutual informations, modularity, description length — and the experiment is how it reproduces: clonal, sex by uniform swapping, or two sexes each passing on one half of the genome intact. Watch the matrix sort itself into blocks, the genome's entropy fall as it makes up its mind, and the planted split and the exact optimum disagree; then switch to the **Lab** and race the three worlds across planted signal strength (survival curves, hypotheses stated next to what was observed) or start them all inside a trap and time the escape; a **Linkage** view correlates every pair of loci across the population, where the three rules separate most sharply — uniform crossover drives the matrix to linkage equilibrium, clonal reproduction keeps it strong, and the two-sex rule keeps the within-half blocks while flattening the cross-half one. (Storeroom.)
