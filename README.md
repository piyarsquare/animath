...
10. **[Trees and Nets](https://piyarsquare.github.io/animath/#/trees-and-nets)** – phylogenetics: a Neighbor-Joining **tree** and a circular split **network** (the "net"), where crossing chords reveal structure no single tree can hold. **Build** (the default) lets you *fatten a tree into a net* — every tree edge is a split, and turning up its **Fatten** slider adds the crossing split, opening a box; you watch the NJ tree, the SplitsTree net, and the generated distance matrix all follow. **Nets** goes the other way: edit a distance matrix directly. **Run** plays Neighbor-Joining and NeighborNet **step by step** — the Q matrix shows *why* each pair joins, and the circular order locks in block by block. A de-emphasized *Fibers* mode keeps the tree-space geometry (the associahedron of triangulations and the cube of orders). Port of the private `quantum-tree`.
