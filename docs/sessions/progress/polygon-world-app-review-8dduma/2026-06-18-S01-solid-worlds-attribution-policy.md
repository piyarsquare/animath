...
### 🟢 code · 13:10 — Solid Worlds: lighting symmetrized over the holonomy point group
**Why:** Dan: the lighting must *respect the world's symmetries* — "if there is
sky lighting we need equal floor lighting for worlds where top and bottom
reverse; the airplane gets inconsistent lighting as it climbs the decks of Klein
world." The 12:48 `bodyLinear` rotation only fixed the *observer's* cell; it
didn't make the whole cover (or stacked decks) consistent.

**Fix — make the lighting genuinely holonomy-invariant** (`coverEngine.ts`):
- Compute the **point group** `G` = the finite group generated by the pairings'
  linear parts (orthogonal → closure terminates; flat cube groups are tiny, ≤ 4).
- **Directional key/fill lights are replicated over the orbit** `{g·dir : g∈G}`,
  each at intensity `1/|G|`. A `max(0, n·l)` sum over an orbit is exactly
  `G`-invariant yet still directional (it doesn't cancel) — every copy of a room
  shades identically and climbing the decks is seamless. Since `bodyLinear ∈ G`,
  it's automatically invariant under the carried frame too → the per-frame
  rotation is gone.
- **Hemisphere:** keep the sky/ground split only when **every** holonomy fixes
  +y; else `ground = sky` (equal floor lighting). Amphicosm/half-turn (up→−up) and
  quarter-turn (up→horizontal) go symmetric; 3-torus (G = {I}) is unchanged.

### 🟢 code · 12:48 — Solid Worlds: lighting rides the developing frame *(superseded by 13:10)*
