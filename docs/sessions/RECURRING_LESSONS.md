...
| **L2** | Build the full feature, then ask — the maximal/wrong-reading version finished before a cheap "how far / which meaning?" check; 3–5 build-revert cycles | ~9 | 🟡 Promoted (rule) | `BUILDING_AN_APP.md` top callout ("Before you build … pin scope and get a reference") | Not automatable; relies on the build-flow reflex + three-hats reviews |
