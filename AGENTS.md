...
branch name, `claude/` stripped) so parallel branches don't collide — sharing
`report.css` + `report.js` and indexed by `docs/sessions/index.html` (regenerated by
`node docs/sessions/build-index.mjs`). GitHub shows `.html` as source; open locally
for the full view. See CLAUDE.md → "Agent session skills" for details.
