...
import { balancedTreeEdges, fattenedMetric } from './lib/fattenTree';
...
import { FattenControls } from './views/BuildPanel';
...
export default function TreesAndNets(): JSX.Element {
  ...
  // Changing n or the preset resets the matrix — but only outside Build mode,
  // where the matrix is generated from the fatten controls (a separate effect).
  useEffect(() => {
    setHl(null);
    if (appModeRef.current !== 'build') setMatrix(preset(n, presetName));
  }, [n, presetName]);
  ...
  const [appMode, setAppMode] = usePersistentState<'nets' | 'fibers' | 'run' | 'build'>(`${APP_ID}:appmode`, 'build');
  ...
  // ---- Build mode: fatten a tree into a CDM (the inverse of the app) ----
  // The leaves sit in a fixed cyclic order; the starting tree's internal edges
  // are non-crossing arcs. Fatten an edge and a *crossing* split is added,
  // opening a box — the tree becomes a net. The matrix is GENERATED from these
  // weights, so while in Build mode a single effect drives `matrix`.
  const [branchLen, setBranchLen] = usePersistentState<number>(`${APP_ID}:branchlen`, 1);
  const [leafLen, setLeafLen] = usePersistentState<number>(`${APP_ID}:leaflen`, 1);
  const [fatten, setFatten] = usePersistentState<Record<string, number>>(`${APP_ID}:fatten`, {});
  ...
  useEffect(() => {
    ...
    setMatrix(fattenedMetric(n, treeEdges, { branchLen, leafLen, fatten }));
  }, [appMode, n, treeEdges, branchLen, leafLen, fatten]);
  const setFattenAt = useCallback((key: string, v: number) => {
    setFatten((prev) => ({ ...prev, [key]: Math.max(0, v) }));
  }, [setFatten]);
  const resetFatten = useCallback(() => setFatten({}), [setFatten]);
  ...
  // ---- Build mode: shape a tree, fatten its edges, see the generated tree + net ----
  const buildSections: SectionDef[] = [
    {
      id: 'tree', title: 'Tree', arch: 'subject', estHeight: 200,
      node: (
        <div style={{ display: 'grid', gap: 10 }}>
          <Pills<number> label="Leaves (n)" value={n} onChange={setN} options={[5, 6, 7, 8, 9].map((k) => ({ value: k, label: String(k) }))} />
          <Slider label="Branch length" value={branchLen} min={0} max={2} step={0.1} onChange={setBranchLen} format={(v) => v.toFixed(1)} />
          <Slider label="Leaf length" value={leafLen} min={0} max={2} step={0.1} onChange={setLeafLen} format={(v) => v.toFixed(1)} />
          <Kicker>A balanced tree on n leaves. The distances it implies feed the tree and the net — now
            <b> fatten</b> its internal edges below to turn the tree into a CDM (a net).</Kicker>
        </div>
      ),
    },
    {
      id: 'fatten', title: 'Fatten edges', arch: 'drive', estHeight: 250,
      node: (
        <div style={{ display: 'grid', gap: 10 }}>
          <FattenControls edges={treeEdges} fatten={fatten} highlight={hl} onFatten={setFattenAt} onSelect={setHl} />
          <button type="button" onClick={resetFatten}
          ...
          <Kicker>Each edge starts crisp (a tree split). Fatten it and the <b>crossing</b> split is added — a
            box opens, so the tree becomes a net. The NJ tree keeps the dominant tree; the SplitsTree net shows the boxes.</Kicker>
        </div>
      ),
    },
    { id: 'connect', title: 'Selection', arch: 'readout', estHeight: 130, node: connectNode },
  ...
  const buildViews: ViewDef[] = [
    { id: 'njtree', title: 'Neighbor-Joining tree (splits)', defaultRect: { x: 360, y: 16, w: 400, h: 380 }, node: <NJTreeView nj={nj} matrix={matrix} highlight={hl} onSelect={setHl} /> },
    { id: 'splitgraph', title: 'Split network (SplitsTree)', defaultRect: { x: 776, y: 16, w: 440, h: 380 }, node: <SplitGraphView graph={splitGraph} matrix={matrix} highlight={hl} onSelect={setHl} /> },
    {
      id: 'genmatrix', title: 'Generated distances', defaultRect: { x: 360, y: 412, w: 360, h: 300 },
      node: (
  ...
  const layouts: LayoutDef[] =
    appMode === 'nets'
      ...
      : appMode === 'fibers'
        ...
        : appMode === 'build'
          ? [{ id: 'essentials', name: 'Build', open: { tree: { x: 84, y: 16 }, fatten: { x: 84, y: 232 } }, views: { njtree: { open: true }, splitgraph: { open: true }, genmatrix: { open: true } } }]
          : [{ id: 'essentials', name: 'Run', open: { run: { x: 84, y: 16 }, qmatrix: { x: 84, y: 200 }, runstate: { x: 84, y: 446 } }, views: { run: { open: true } } }];

  // Build is the front door (fatten a tree → net); Nets edits a matrix directly;
  // Run watches the algorithms; Fibers (the abstract tree-space explorer) stays
  // last — reachable, but no longer front-and-center.
