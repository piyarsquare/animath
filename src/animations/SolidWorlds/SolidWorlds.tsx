...
import { analyzeSolid } from './solidSchema';
import { LOOKS } from './looks';
import { makeCoverEngine } from './coverEngine';
import {
  EngineDeps3, SolidEngine, ChiralityState,
...
export default function SolidWorlds() {
  ...
  const viewNode = (
    <>
      ...
      <Slider label="Cover depth" value={coverDepth} min={0} max={2} step={1} onChange={(v) => setCoverDepth(Math.round(v))} format={(v) => `${Math.round(v)}`} />
