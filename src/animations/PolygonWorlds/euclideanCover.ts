...
export const L = 30;   // fundamental-square side in world units
 ...
 */
export function makeEuclideanCover(spec: WorldSpec): CoverModel {
  ...
  function visibleSquares(): SquarePlacement[] {
    ...
    for (let di = -K; di <= K; di++) {
      for (let dj = -K; dj <= K; dj++) {
        ...
        S.makeScale(1, 1, sz);
        const m = new THREE.Matrix4().multiplyMatrices(M.makeTranslation(I * L, 0, J * L), S);
