...
export function Trace({ history, optimum, plantedScore, units }: Props) {
  if (!history.length) return <div className="sd-empty">No generations yet.</div>;
  const k = Math.max(1, Math.ceil(history.length / 600));
  const pts = history.filter((_, i) => i % k === 0);
  if (pts[pts.length - 1] !== history[history.length - 1]) pts.push(history[history.length - 1]);
  const xMax = Math.max(10, history[history.length - 1].gen);
  ...
  return (
    <div className="sd-trace">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="sd-chart">
        ...
        <path className="ln mean" d={path(pts, p => p.mean, xMax, fMin, fMax)} />
        <path className="ln best" d={path(pts, p => p.best, xMax, fMin, fMax)} />
      </svg>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="sd-chart">
        <path className="ln neut" d={path(pts, p => p.neutral, xMax, 0, eMax)} />
        <path className="ln round" d={path(pts, p => p.rounded, xMax, 0, eMax)} />
        <path className="ln samp" d={path(pts, p => p.entropy, xMax, 0, eMax)} />
      </svg>
