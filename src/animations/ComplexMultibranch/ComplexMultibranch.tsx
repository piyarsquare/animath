...
function complexSqrtBranch(z: THREE.Vector2, branch: number): THREE.Vector2 {
  const r = Math.hypot(z.x, z.y);
  const t = Math.atan2(z.y, z.x);
  const sr = Math.sqrt(r);
  const angle = t * 0.5 + branch * Math.PI;
  return new THREE.Vector2(sr * Math.cos(angle), sr * Math.sin(angle));
}
...
function complexLnBranch(z: THREE.Vector2, branch: number): THREE.Vector2 {
  const r = Math.hypot(z.x, z.y);
...
function complexBranchSqrtPoly(z: THREE.Vector2, branch: number): THREE.Vector2 {
...
function applyComplex(z: THREE.Vector2, t: number, branch: number): THREE.Vector2 {
  switch (t) {
    ...
    case 14: return complexBranchSqrtPoly(z, branch);
