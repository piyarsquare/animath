...
/* plane is 'XY' .. 'UV'; θ positive = +90°. Consumers apply these as the 4D
 * rotation p ↦ L · p · conj(R), under which each entry rotates exactly the named
 * plane (fixing the other two axes). Imaginary–imaginary planes (XY, XU, YU) use
 * L = R; planes that include the v (scalar) axis (XV, YV, UV) use R = conj(L). */
